...
import {
  authorisationUrl as metaAuthorisationUrl,
  MetaNotConfiguredError,
} from "../../modules/oauth/meta.service.js";
import {
  authorisationUrl as instagramAuthorisationUrl,
...
export function registerSocialCommands(bot: Telegraf) {
  ...
  bot.command(commandTrigger("connect"), async (ctx) => {
    ...
    // With an app id and secret configured, this is a link to tap — no token is
    // generated by hand, none is pasted into a chat, and the one stored doesn't
    // expire. The manual form below stays for when those aren't set.
    if (parts.length === 0) {
      // Instagram Login wins wherever it's configured: it reaches the account
      // directly, with no Page, portfolio or role in between to be broken.
      try {
        await ctx.reply(
          [
            "CONNECT INSTAGRAM",
            "",
            "Open this and log in with Instagram — no Facebook Page involved:",
            "",
            instagramAuthorisationUrl(),
            "",
            "The link expires in 10 minutes. Nothing to copy — I collect the token myself and renew it before it runs out.",
            "",
            "Then /ready to check it works.",
          ].join("\n"),
        );
        return;
      } catch (error) {
        if (!(error instanceof InstagramNotConfiguredError)) throw error;
        // Fall through to the Page-based flow below.
      }
      ...
      try {
        await ctx.reply(
          [
            "CONNECT INSTAGRAM",
            "",
            "Open this and approve — pick the Page your Instagram account is attached to:",
            "",
            metaAuthorisationUrl(),
            "",
            "The link expires in 10 minutes. You won't have to copy anything: I collect the token myself, and " +
              "the one I keep doesn't expire.",
            "",
            "Then /ready to check it works.",
          ].join("\n"),
        );
        ...
        await ctx.reply(
          `${error.message}\n\nUntil then you can connect by hand: /connect <instagram-account-id> <access-token>`,
        );
        return;
      }
    }
